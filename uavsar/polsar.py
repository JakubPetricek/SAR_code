...
def get_polsar_stack(in_dir, polarization='L', bounds = False):
    """
    ...
    desc : dict with the contents of the annotation file
    """
    ...
    if len(fps) == 0:
        ...
        # Read GRD files
        for f in fps:
            #HERE I DID NOT TREAT THE FILE NAME BASED ON POLARIZATION 
...
def calc_C3(HHHH, HHHV, HVHV, HVVV, HHVV, VVVV):
    ...
    Formula derived from Cloude and Pottier, 1997 
...
def H_A_alpha_decomp(in_dir, out_dir, polarization, parralel = False):
    """
    in_dir must have all polarizations of []
    out_dir - must exist
    pol - polarization state (L or P) to correctly parse the filenames
    """
    log.info('Collecting polsar stack')
    stack, desc = get_polsar_stack(in_dir,polarization)
    ...
    for name, arr in d.items():
        ...
        array_to_tiff(arr, out_fp +'.tif', desc = desc, type = 'grd_pwr')
